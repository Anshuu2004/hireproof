...
| Bias / fairness audit (four-fifths / NYC LL144) | **Real (small-N)** — runs over real scored sessions using **opt-in, aggregate-only** demographics **plus** proxy cohorts (language); groups below the minimum cell size are suppressed; emits an Ed25519-signed, did:web-verifiable certificate (`/fairness`, `/api/bias-audit/run`). Larger-N needs more real outcome data, which we don't fabricate. |
